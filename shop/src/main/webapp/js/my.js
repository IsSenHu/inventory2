...
function registUser() {
    ...
    $.ajax({
        type : 'post',
        url : '/bossRegist.do',
        data : 'valiCode=' + valiCode + '&password=' + password + '&phone=' + mobilePhone,
        success : function (data) {
            var json = $.parseJSON(data);
...
function userLogin() {
    ...
    $.ajax({
        ...
        success : function (data) {
            var json = $.parseJSON(data);
            if(json.isAuthenticate){
                if(json.isAuthenticate == 'noAuthenticate'){
                    location.href = '/toOne.do';
                }else if(json.isAuthenticate == 'authenticateing'){
                    location.href = '/toFouth.do';
                }else if(json.isAuthenticate == 'haveAuthenticate'){
