                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <div class="col-xs-2">
                                    <input type="text" class="form-control" placeholder="请输入发货单号" name="deliverOrderId">
                                </div>
                                <div class="col-xs-2">
                                    <input type="text" class="form-control" placeholder="请输入用户Id" name="userId">
                                </div>
                                <div class="col-xs-2">
                                    <input type="text" class="form-control" placeholder="请输入店铺Id" name="storesId">
                                </div>
                                <div class="col-xs-1" style="text-align: right;">
                                    <select name="statu" class="form-control">
                                        <option value="">发货单状态</option>
                                        <option value="deliverNoDone">未处理</option>
                                        <option value="deliverDelived">发货</option>
                                    </select>
                                </div>
                                <div class="col-xs-2">
                                </div>
                            ...
                            <div class="col-xs-12">
                                <table id="simple-table" class="table  table-bordered table-hover" style="text-align: center;">
                                    ...
                                    <tr>
                                        ...
                                        <th style="width:15%; text-align: center;">合计/元</th>
                                        <th style="width:15%; text-align: center;">用户编号</th>
                                        <th style="width:15%; text-align: center;">店铺编号</th>
                                    ...
                                    <tbody>
                                        <c:forEach items="${page.content}" var="vo">
                                            <tr>
                                                <td>${vo.deliverOrder.deliverOrderId}</td>
                                                <td>${vo.deliverOrder.totalMoney}元</td>
                                                <td>${vo.deliverOrder.userId}</td>
                                                <td>${vo.deliverOrder.storesId}</td>
                                                <td>${vo.deliverOrder.orderId}</td>
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${vo.deliverOrder.statu == 'deliverNoDone'}">
                                                            发货单未处理
                                                        </c:when>
                                                        <c:when test="${vo.deliverOrder.statu == 'deliverDelived'}">
                                                            发货
                                                        </c:when>
                                                        <c:otherwise>
                                                            取消发货
                                                ...
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${vo.deliverOrder.statu == 'deliverNoDone'}">
                                                            <button value="${vo.deliverOrder.deliverOrderId}" class="access" style="width: 66px;height: 32px; border-radius: 6px; background-color: green; color: #FFFFFF;">发货</button>
                                                        </c:when>
                                                        <c:otherwise>
                                                            无可用操作
                                                        </c:otherwise>
                                            ...
                                            <tr>
                                                <td colspan="6">
                                                    <table class="table table-bordered" style="margin-top: -8px; background-color: skyblue;">
                                                        <tr>
                                                            <th style="width:35%; text-align: center;">地址</th>
                                                            <th style="width:10%; text-align: center;">联系人</th>
                                                            <th style="width:35%; text-align: center;">电话</th>
                                                            <th style="width:20%; text-align: center;">发货时间</th>
                                                        </tr>
                                                            <tr>
                                                                <td style="text-align: center;">${vo.address.address}</td>
                                                                <td style="text-align: center;">${vo.address.receivePersonName}</td>
                                                                <td style="text-align: center;">${vo.address.phone}</td>
                                                                <td style="text-align: center;">
                                                                    <c:choose>
                                                                        ...
                                                                        <c:when test="${vo.deliverOrder.statu == 'deliverDelived'}">
                                                                            <fmt:formatDate value="${vo.deliverOrder.deliverTime}" pattern="yyyy年MM月dd日 HH时mm分ss秒"></fmt:formatDate>
                                                                        </c:when>
                                                                        <c:otherwise>
                                                                            已取消发货
                                                                        </c:otherwise>
                                            ...
                                            <tr>
                                                <td colspan="6">
                                                    <table class="table table-bordered" style="margin-top: -8px; background-color: skyblue;">
                                                        ...
                                                        <c:forEach items="${vo.deliverOrder.rows}" var="row">
                                                            <tr>
                    ...
                    <ul class="pagination">
                        <c:choose>
                            <c:when test="${page.currentPage == 1}">
                                <li><a href="${pageContext.request.contextPath}/findDeliverOrder.action?deliverOrderId=${fd.deliverOrderId}&userId=${fd.userId}&storesId=${fd.storesId}&statu=${fd.statu}&currentPage=1">首页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findDeliverOrder.action?deliverOrderId=${fd.deliverOrderId}&userId=${fd.userId}&storesId=${fd.storesId}&statu=${fd.statu}&currentPage=${page.currentPage + 1}">下一页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findDeliverOrder.action?deliverOrderId=${fd.deliverOrderId}&userId=${fd.userId}&storesId=${fd.storesId}&statu=${fd.statu}&currentPage=${page.totalPage}">尾页</a></li>
                            </c:when>
                            <c:when test="${page.currentPage == page.totalPage}">
                                <li><a href="${pageContext.request.contextPath}/findDeliverOrder.action?deliverOrderId=${fd.deliverOrderId}&userId=${fd.userId}&storesId=${fd.storesId}&statu=${fd.statu}&currentPage=1">首页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findDeliverOrder.action?deliverOrderId=${fd.deliverOrderId}&userId=${fd.userId}&storesId=${fd.storesId}&statu=${fd.statu}&currentPage=${page.currentPage - 1}">上一页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findDeliverOrder.action?deliverOrderId=${fd.deliverOrderId}&userId=${fd.userId}&storesId=${fd.storesId}&statu=${fd.statu}&currentPage=${page.totalPage}">尾页</a></li>
                            </c:when>
                            <c:otherwise>
                                <li><a href="${pageContext.request.contextPath}/findDeliverOrder.action?deliverOrderId=${fd.deliverOrderId}&userId=${fd.userId}&storesId=${fd.storesId}&statu=${fd.statu}&currentPage=1">首页</a></li>
...
<script type="text/javascript">
    ...
    $("#find").click(function () {
        var deliverOrderId = $("input[name='deliverOrderId']").val();
        var userId = $("input[name='userId']").val();
        var storesId = $("input[name='storesId']").val();
        var statu = $("select[name='statu']").val();
        var data = 'deliverOrderId=' + deliverOrderId + '&userId=' + userId + '&storesId=' + storesId + '&statu=' + statu;
        location.href = '${pageContext.request.contextPath}/findDeliverOrder.action?' + data + '&currentPage=${page.currentPage}';
    });
    /*
    * 发货
    * */
    $(".access").click(function () {
        var deliverOrderId = $(this).val();
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/deliverGoods.action',
            data : 'deliverOrderId=' + deliverOrderId,
    ...
    * */
    $(".faile").click(function () {
        ...
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/cancelDeliver.action',
            data : 'deliverOrderId=' + deliverOrderId,
            success : function (data) {
                var json = $.parseJSON(data);
                if(json.code == 200){
