                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <div class="col-xs-2">
                                    <input type="text" class="form-control" placeholder="请输入编号" name="outOrderId">
                                </div>
                                <div class="col-xs-4">
                                    <div class="col-xs-1" style="text-align: right;">起始</div>
                                    <div class="col-xs-5">
                                        <input type="date" class="form-control" name="start">
                                    </div>
                                    <div class="col-xs-1" style="text-align: right;">截至</div>
                                    <div class="col-xs-5">
                            ...
                            <div class="col-xs-12">
                                <table id="simple-table" class="table  table-bordered table-hover" style="text-align: center;">
                                    ...
                                    <tr>
                                        ...
                                        <th style="width:15%; text-align: center;">生成时间</th>
                                        <th style="width:15%; text-align: center;">总金额/元</th>
                                        <th style="width:15%; text-align: center;">状态</th>
                                        <th style="width:10%; text-align: center;">操作</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                        <c:forEach items="${page.content}" var="outOrder">
                                            <tr>
                                                <td>${outOrder.outOrderId}</td>
                                                <td><c:out value="${outOrder.orderId}"></c:out></td>
                                                <td><fmt:formatDate value="${outOrder.outTime}" pattern="yyyy年MM月dd日 HH时mm分ss秒"></fmt:formatDate></td>
                                                <td>${outOrder.totalMoney}</td>
                                                <td>
                                                    <c:if test="${outOrder.inStatu.value == 0}">
                                                        未审核
                                                    </c:if>
                                                    <c:if test="${outOrder.inStatu.value == 1}">
                                                        审核未通过
                                                    </c:if>
                                                    <c:if test="${outOrder.inStatu.value == 2}">
                                                        审核通过
                                                    </c:if>
                                                </td>
                                                <td>
                                                    <c:if test="${outOrder.inStatu.value == 0}">
                                                        <button value="${outOrder.outOrderId}" class="access" style="width: 66px;height: 32px; border-radius: 6px; background-color: green; color: #FFFFFF;">通过</button>
                                                        <button value="${outOrder.outOrderId}" class="faile" style="width: 66px;height: 32px; border-radius: 6px; background-color: red; color: #FFFFFF;">失败</button>
                                                    </c:if>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td colspan="6">
                                                    <table class="table table-bordered" style="margin-top: -8px; background-color: skyblue;">
                                                        <tr>
                                                            <th style="width:20%; text-align: center;">物品编号</th>
                                                            ...
                                                            <th style="width:20%; text-align: center;">总价</th>
                                                        </tr>
                                                        <c:forEach items="${outOrder.rows}" var="row">
                                                            <tr>
                                                                <td style="text-align: center;">${row.item.itemId}</td>
                                                                <td style="text-align: center;">${row.item.itemName}</td>
                                                                <td style="text-align: center;">${row.money}</td>
                                                                <td style="text-align: center;">${row.number}</td>
                                                                <td style="text-align: center;">${row.rowMoney}</td>
                                                            </tr>
                                                        </c:forEach>
                                                    </table>
                                                </td>
                    ...
                    <ul class="pagination">
                        <c:choose>
                            <c:when test="${page.currentPage == 1}">
                                <li><a href="${pageContext.request.contextPath}/pageNoVerfyOut.action?currentPage=1">首页</a></li>
                                <li><a href="${pageContext.request.contextPath}/pageNoVerfyOut.action?currentPage=${page.currentPage + 1}">下一页</a></li>
                                <li><a href="${pageContext.request.contextPath}/pageNoVerfyOut.action?currentPage=${page.totalPage}">尾页</a></li>
                            </c:when>
                            <c:when test="${page.currentPage == page.totalPage}">
                                <li><a href="${pageContext.request.contextPath}/pageNoVerfyOut.action?currentPage=1">首页</a></li>
                                <li><a href="${pageContext.request.contextPath}/pageNoVerfyOut.action?currentPage=${page.currentPage - 1}">上一页</a></li>
                                <li><a href="${pageContext.request.contextPath}/pageNoVerfyOut.action?currentPage=${page.totalPage}">尾页</a></li>
                            </c:when>
                            <c:otherwise>
                                <li><a href="${pageContext.request.contextPath}/pageNoVerfyOut.action?currentPage=1">首页</a></li>
            ...
            <c:if test="${page.rowsTotal > 10}">
                <center>
                    <ul class="pagination">
                        <c:choose>
                            <c:when test="${page.currentPage == 1}">
                                <li><a href="${pageContext.request.contextPath}/findNoVerfyOut.action?outOrderId=${outOrderId}&start=${start}&end=${end}&outStatuValue=${outStatuValue}&currentPage=1">首页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findNoVerfyOut.action?outOrderId=${outOrderId}&start=${start}&end=${end}&outStatuValue=${outStatuValue}&currentPage=${page.currentPage + 1}">下一页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findNoVerfyOut.action?outOrderId=${outOrderId}&start=${start}&end=${end}&outStatuValue=${outStatuValue}&currentPage=${page.totalPage}">尾页</a></li>
                            </c:when>
                            <c:when test="${page.currentPage == page.totalPage}">
                                <li><a href="${pageContext.request.contextPath}/findNoVerfyOut.action?outOrderId=${outOrderId}&start=${start}&end=${end}&outStatuValue=${outStatuValue}&currentPage=1">首页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findNoVerfyOut.action?outOrderId=${outOrderId}&start=${start}&end=${end}&outStatuValue=${outStatuValue}&currentPage=${page.currentPage - 1}">上一页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findNoVerfyOut.action?outOrderId=${outOrderId}&start=${start}&end=${end}&outStatuValue=${outStatuValue}&currentPage=${page.totalPage}">尾页</a></li>
                            </c:when>
                            <c:otherwise>
                                <li><a href="${pageContext.request.contextPath}/findNoVerfyOut.action?outOrderId=${outOrderId}&start=${start}&end=${end}&outStatuValue=${outStatuValue}&currentPage=1">首页</a></li>
...
<script type="text/javascript">
    ...
    $("#find").click(function () {
        ...
        var data = 'outOrderId=' + outOrderId + '&start=' + start + '&end=' + end;
        location.href = '${pageContext.request.contextPath}/findNoVerfyOut.action?' + data + '&currentPage=${page.currentPage}';
    });
    ...
    $(".access").click(function () {
        ...
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/outAccess.action',
            ...
            success : function (data) {
                ...
                if(json.error){
                    ...
                    alert("审核成功！");
                    location.href = '${pageContext.request.contextPath}/pageNoVerfyOut.action?currentPage=${page.currentPage}';
                }else if(json.faile) {
                    alert("库存不足，不能通过");
                }
            },
    ...
    $(".faile").click(function () {
        ...
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/outFaile.action',
            data : 'outOrderId=' + outOrderId,
            success : function (data) {
                var json = $.parseJSON(data);
                if(json.error){
