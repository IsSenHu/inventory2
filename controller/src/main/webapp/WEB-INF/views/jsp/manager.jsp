                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <table id="simple-table" class="table  table-bordered table-hover" style="text-align: center;">
                                    <thead>
                                        <tr>
                                            <th style="width:15%; text-align: center;">帐号</th>
                                            <th style="width:15%; text-align: center;">真实姓名</th>
                                        ...
                                        <c:forEach items="${page.content}" var="user">
                                            <c:if test="${user.userId != 1}">
                                                <tr>
                                                    ...
                                                    <td><fmt:formatDate value="${user.createDate}" pattern="yyyy年MM月dd日 HH时mm分ss秒"></fmt:formatDate></td>
                                                    <td>
                                                        <c:if test="${user.userStatu.value == 1}">启用</c:if>
                                                        <c:if test="${user.userStatu.value == 2}">禁用</c:if>
                                                    </td>
                                                    <td>
                                                        <div class="hidden-sm hidden-xs btn-group">
                                                            <button value="${user.userId}" onclick="showRoles(this)" style="width: 126px;height: 32px; border-radius: 6px; background-color: #438EB9; color: #FFFFFF;">角色管理</button>
                                                            <c:if test="${user.userStatu.value == 1}">
                                                                <button style="width: 66px;height: 32px; border-radius: 6px; background-color: #D15B47; color: #FFFFFF;" value="${user.userId}" onclick="updateStatu(this)">禁用</button>
                                                            </c:if>
                                                            <c:if test="${user.userStatu.value == 2}">
                                                                <button style="width: 66px;height: 32px; border-radius: 6px; background-color: #D15B47; color: #FFFFFF;" value="${user.userId}" onclick="updateStatu(this)">启用</button>
                                                            </c:if>
                                                            <a href="${pageContext.request.contextPath}/deleteManager.action?userId=${user.userId}&currentPage=${page.currentPage}">
...
<script type="text/javascript">
    ...
    function updateStatu(obj) {
        var userId = obj.value;
        console.log(userId);
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/updateManagerStatu.action',
            data : 'userId=' + userId,
            success : function (data) {
                ...
                if(json.name == 'error'){
                    ...
                    alert("操作成功");
                    location.href = '${pageContext.request.contextPath}/pageManager.action?currentPage=${page.currentPage}';
                }
            },
            error : function (data) {
                alert("发生错误{error}");
    ...
    function showRoles(obj) {
        ...
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/showRoles.action',
            data : 'userId=' + userId,
            success : function (data) {
                var json = $.parseJSON(data);
                console.log(json);
                ...
                $("#my").empty();
                var myRoles = json.p;
                for(index in myRoles){
                    var role = myRoles[index];
                    var p = '<p><span class="dss">' + role.roleName + '</span></p>';
                    $("#my").append(p);
                }
                var allRoles = json.u;
                for(i in allRoles){
                    var role = allRoles[i];
                    var l = '<label><input name="roles" type="checkbox" value="' + role.roleId + '">' + role.roleName + '</label><br/>';
                    $("#all").append(l);
                }
    ...
    function updatelss() {
        ...
        var roleStr = '';
        if(roles.length >= 2){
            roleStr = roles.substr(0, roles.length - 1);
            console.log(roleStr);
        }
        ...
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/updateUserRole.action',
            data : 'userId=' + userId + '&roleStr=' + roleStr,
            success : function (data) {
                var json = $.parseJSON(data);
                if(json.name == 'error'){
                    alert("发生错误{noInput}");
