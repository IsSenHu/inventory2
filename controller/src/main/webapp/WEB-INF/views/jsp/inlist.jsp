                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                ...
                                <div class="col-xs-1" style="text-align: left;">
                                    <input id="addbuttom" style="width: 66px;height: 32px; border-radius: 6px; background-color: #D15B47; color: #FFFFFF;" type="button" value="添加">
                                </div>
                            </div>
                        </div>
                        <hr>
                                            ...
                                            <tr style="height: 26px;">
                                                ...
                                                <td>
                                                    <div class="hidden-sm hidden-xs btn-group">
                                                        <c:if test="${inOrder.inStatu.value == 0}">
                                                            <a href="${pageContext.request.contextPath}/deleteIn.action?inOrderId=${inOrder.inOrderId}">
            ...
            <c:if test="${page.rowsTotal > 10}">
                <center>
                    <ul class="pagination">
                        <c:choose>
                            <c:when test="${page.currentPage == 1}">
                                <li><a href="${pageContext.request.contextPath}/findIn.action?inOrderId=${inOrderId}&itemName=${itemName}&currentPage=1">首页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findIn.action?inOrderId=${inOrderId}&itemName=${itemName}&currentPage=${page.currentPage + 1}">下一页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findIn.action?inOrderId=${inOrderId}&itemName=${itemName}&currentPage=${page.totalPage}">尾页</a></li>
                            </c:when>
                            <c:when test="${page.currentPage == page.totalPage}">
                                <li><a href="${pageContext.request.contextPath}/findIn.action?inOrderId=${inOrderId}&itemName=${itemName}&currentPage=1">首页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findIn.action?inOrderId=${inOrderId}&itemName=${itemName}&currentPage=${page.currentPage - 1}">上一页</a></li>
                                <li><a href="${pageContext.request.contextPath}/findIn.action?inOrderId=${inOrderId}&itemName=${itemName}&currentPage=${page.totalPage}">尾页</a></li>
                            </c:when>
                            <c:otherwise>
                                <li><a href="${pageContext.request.contextPath}/findIn.action?inOrderId=${inOrderId}&itemName=${itemName}&currentPage=1">首页</a></li>
...
<script type="text/javascript">
    ...
    * */
    $("#addbuttom").click(function () {
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/showAllSport.action',
            success : function (data) {
                var json = $.parseJSON(data);
                for (var index in json){
                    var option = '<option value="' + json[index].sportId + '">' + json[index].sportName + '</option>';
                    $("select[name='sportId']").append(option);
                }
            }
        });
        $("#selectSport").css("display", "block");
    });
    /*
    * 选择完运动后打开添加入库单的框
    * */
    $("#toAddIn").click(function () {
        //获得所选择的运动
        var sportId = $("select[name='sportId']").val();
        ...
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/toIn.action',
            data : 'sportId=' + sportId,
            success : function (data) {
                var json = $.parseJSON(data);
                for(var index in json){
                    var option = '<option value="' + json[index][0] + '">' + json[index][1] + '</option>';
                    $("select[name='itemId']").append(option);
                }
                $("#add").css("display", "block");
            }
        });
    ...
    $("#addIn").click(function () {
        ...
        var itmeNumber = $("input[name='itmeNumber']").val();
        ...
        $.ajax({
            type : 'post',
            url : '${pageContext.request.contextPath}/addIn.action',
            data : 'itemId=' + itemId + '&itmeNumber=' + itmeNumber + '&totalMoney=' + totalMoney,
            success : function (data) {
                var json = $.parseJSON(data);
                if(json.error){
                    alert("请确认输入的数据！");
                    ...
                    alert("添加成功！");
                    location.href = '${pageContext.request.contextPath}/pageIn.action?currentPage=' + ${page.currentPage};
                }
            }
        });
    });
    $("#findIn").click(function () {
        ...
        location.href = '${pageContext.request.contextPath}/findIn.action?inOrderId=' + inOrderId + '&itemName=' + itemName + '&currentPage=' + ${page.currentPage};
    });
</script>
